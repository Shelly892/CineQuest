...
# 1. Create a rating
echo "1. Creating a rating for user 'user3' and movie '550'"
...
echo "2. Creating another rating for user 'user3' and movie '680'"
curl -X POST http://localhost:3003/api/ratings \
  -H "Content-Type: application/json" \
  -H "X-User-Id: user3" \
  -H "X-User-Email: user3@example.com" \
...
echo "3. Getting rating for user 'user3' and movie '550'"
curl -X GET "http://localhost:3003/api/ratings?userId=user3&movieId=550"
echo -e "\n"

# 4. Get all ratings by a user
echo "4. Getting all ratings by user 'user3'"
curl -X GET "http://localhost:3003/api/ratings/all?userId=user3&page=0&size=20"
echo -e "\n"
...
# 7. Update a rating
echo "7. Updating rating for user 'user3' and movie '550'"
...
# 10. Delete a rating
echo "10. Deleting rating for user 'user3' and movie '680', movie '550'"
